...
var (
	...
	output          = flag.String("output", "", "output file name; default srcdir/<type>_enumerx.go")
	transformMethod = flag.String("transform", "noop", "enum item name transformation method. Options: [ noop | snake | kebab | snake_upper | kebab_upper ]")
	trimPrefix      = flag.String("trimprefix", "", "transform each item name by removing a prefix. Default: \"\" Can use vars like: @type")
...
func Usage() {
	...
	fmt.Fprintf(os.Stderr, "\thttps://github.com/niktri/enumerx\n")
...
func main() {
	...
	g.Printf("// Code generated by \"enumerx %s\"; DO NOT EDIT.\n", strings.Join(os.Args[1:], " "))
	g.Printf("\n")
	g.Printf("// See https://github.com/niktri/enumerx\n")
	g.Printf("\n")
	g.Printf("// %s\n", comments.String())
	...
	outputName := *output
	if outputName == "" {
		baseName := fmt.Sprintf("%s_enumerx.go", types[0])
		outputName = filepath.Join(dir, strings.ToLower(baseName))
	}

	// Write to tmpfile first
	tmpName := fmt.Sprintf("%s_enumerx_", filepath.Base(types[0]))
...
func (g *Generator) transformValueNames(values []Value, transformMethod string) {
	...
	for i := range values {
		values[i].name = name.Delimit(values[i].name, sep)
		if upper {
